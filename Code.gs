...
function doPost(e) {
  try {
    ...
    const lineCounts = getExistingLineCounts(sheet, part.fecha);
    const rows = buildRows(part, monthName, albaranNumber, lineCounts);
...
function validatePart(part) {
  ...
  if (!part.tecnicos.length) return "Debe indicarse al menos un tÃ©cnico.";
  if (part.tecnicos.some((tecnico) => !tecnico.codigo || !tecnico.nombre || !tecnico.dni)) return "Cada tÃ©cnico debe tener cÃ³digo, nombre y DNI.";
  if (!part.cliente) return "El cliente es obligatorio.";
  if (!part.jefeObra) return "El jefe de obra es obligatorio.";
  if (parseTimeToMinutes(part.horaInicio) === null) return "La hora de entrada no es vÃ¡lida.";
  if (parseTimeToMinutes(part.horaFinal) === null) return "La hora de salida no es vÃ¡lida.";
  ...
  if (toNumber(part.horasJornada) > 8) return "Las horas de jornada no pueden ser mÃ¡s de 8.";
  if (!part.descripcion) return "La descripciÃ³n de trabajo es obligatoria.";
...
function getNextAlbaranNumber(sheet, monthName) {
  ...
  const albaranColumnIndex = headerRow.indexOf("NÂºAlbarÃ¡n") + 1;
...
function getTechnicianLineKey(tecnico) {
  const codigo = cleanText(tecnico && tecnico.codigo, 20);
  if (codigo) return "codigo:" + codigo;
  const nombre = cleanText(tecnico && tecnico.nombre, 120).toUpperCase();
  return nombre ? "nombre:" + nombre : "";
}

function getExistingLineCounts(sheet, fecha) {
  const counts = {};
  if (sheet.getLastRow() < 2) return counts;

  const columnCount = Math.max(sheet.getLastColumn(), SHEET_HEADERS.length);
  const headerRow = sheet.getRange(1, 1, 1, columnCount).getValues()[0];
  const fechaColumnIndex = headerRow.indexOf("Fecha Trabajo");
  const codigoColumnIndex = headerRow.indexOf("CÃ³digo Operario");
  const operarioColumnIndex = headerRow.indexOf("Operario");
  const lineColumnIndex = headerRow.indexOf("Linea ID");
  if (fechaColumnIndex === -1 || (codigoColumnIndex === -1 && operarioColumnIndex === -1)) return counts;

  const values = sheet.getRange(2, 1, sheet.getLastRow() - 1, columnCount).getValues();
  values.forEach((row) => {
    if (String(row[fechaColumnIndex] || "").trim() !== fecha) return;
    const key = getTechnicianLineKey({
      codigo: codigoColumnIndex === -1 ? "" : row[codigoColumnIndex],
      nombre: operarioColumnIndex === -1 ? "" : row[operarioColumnIndex],
    });
    if (!key) return;
    const rawLineId = lineColumnIndex === -1 ? "" : row[lineColumnIndex];
    const lineId = Number(rawLineId);
    const fallbackLineId = (counts[key] || 0) + 1;
    counts[key] = Math.max(counts[key] || 0, Number.isFinite(lineId) && lineId > 0 ? lineId : fallbackLineId);
  });
...
function ensureSheetHeaders(sheet) {
  ...
  const albaranIndex = currentHeaders.indexOf("NÂºAlbarÃ¡n");
  const lineIdIndex = currentHeaders.indexOf("Linea ID");
  const codigoOperarioIndex = currentHeaders.indexOf("CÃ³digo Operario");
  ...
  if (albaranIndex === -1) {
    ...
    currentHeaders.splice(1, 0, "NÂºAlbarÃ¡n");
  }

  if (lineIdIndex === -1) {
    const updatedAlbaranIndex = currentHeaders.indexOf("NÂºAlbarÃ¡n");
    sheet.insertColumnAfter(updatedAlbaranIndex + 1);
    currentHeaders.splice(updatedAlbaranIndex + 1, 0, "Linea ID");
  }
  ...
  if (operarioIndex !== -1 && codigoOperarioIndex === -1) {
    ...
    currentHeaders.splice(updatedOperarioIndex, 0, "CÃ³digo Operario");
  }
...
function buildRows(part, monthName, albaranNumber, lineCounts) {
  const timestamp = new Date().toISOString();
  const declaredHours = getDeclaredHours(part).toFixed(2);
  const realHours = hoursBetween(part.horaInicio, part.horaFinal);

  return part.tecnicos.map((tecnico) => {
    const key = getTechnicianLineKey(tecnico);
    const lineId = (lineCounts[key] || 0) + 1;
    lineCounts[key] = lineId;
